...
We don't use much of Django's templating system, except for the `Administration` and `Processing Nodes` sections. Instead we use Django to expose an [API](/api/task/), which we then tie to a [React.js](https://facebook.github.io/react/) app.
